...
function processImage(inputFile, outputFile) {
    ...
    inputStream.pipe(processBg).pipe(processNt).pipe(processTextLine).pipe(processImageLine).pipe(outputStream);
}
...
function generate(templateName, templateNum, templateIdx) {
    // generate pngs before update tags since there is an issue with text alignment
    generatePngs();
    generateGif();
